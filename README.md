...
## Phase 4: Task 3
The refactoring that I would most like to do is to reduce coupling between classes in the panel package and classes in the model (Project and Task). Each class in the panel package is an individual UI view for different contexts. For example, ProjectView is a UI view for viewing the list of tasks within a project. Currently, each panel class keeps track of its own reference to a Project and/or Task, resulting in high coupling since you can get to Project and Task directly through the reference or through ProjectTrackerUI. ProjectTrackerUI maintains an association with ProjectList, which maintains an association with Project, which maintains an association with Task. If a change to the model were made, then there is a high chance that every panel class will have to be modified to accommodate that change. I would like to refactor the panel classes to access Project and/or Task information through ProjectTrackerUI, and have ProjectTrackerUI keep track of the currently selected project or task, rather than the individual panel classes. 

A related refactoring would be to update existing panel objects rather than create new ones, which might decrease memory usage. Currently, new instances of a panel class are created each time any information that should be displayed is updated. There may be places in ProjectTrackerUI where references to old, unused panel-package objects are kept active. Additionally, since panel classes currently maintain their own references to classes in the model, old versions of model objects may be kept active as a result. Adding functionality to update panel objects and what they display would allow ProjectTrackerUI to only ever need to instantiate one of any panel class. 

Another refactoring I would like to do is to make all methods in the Utilities class static, since there are no methods in Utilities that rely on the state of a Utilities object. This would reduce memory usage as there is no longer any need to make multiple instances of Utilities, with one in each class that is using a Utilities method. 